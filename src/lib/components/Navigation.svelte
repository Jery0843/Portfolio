...
<nav class="nav-shell fixed top-0 left-0 w-full z-[100]" class:scrolled={isScrolled} class:hidden={hidden}>
...
<!-- Mobile Navigation (Outside nav-shell to avoid pointer-events issue) -->
{#if isMenuOpen}
    <div 
        class="mobile-menu"
...
.nav-shell { pointer-events: none; transition: transform 220ms ease; }
.nav-shell.hidden { transform: translateY(-120%); }
...
/* Arc navigation */
.arc-nav { position: relative; padding: 8px 10px; display: flex; gap: 18px; justify-content: center; align-items: center; flex-wrap: wrap; }
.arc-link { position: relative; color: #a7f3d0; text-decoration: none; font-family: monospace; font-size: 0.8rem; letter-spacing: 0.18em; padding: 2px 6px; border-radius: 8px; transition: color 150ms ease, transform 150ms ease; white-space: nowrap; }
.arc-link:hover { color: #34d399; text-shadow: 0 0 12px rgba(52,211,153,0.5); }
.arc-link.active { color: #10b981; text-shadow: 0 0 16px rgba(16,185,129,0.7); }
